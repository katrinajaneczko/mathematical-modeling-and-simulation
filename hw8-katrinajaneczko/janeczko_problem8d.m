...
%   of direction for the agent at hand. Agents within a distance 
%   of 1 of the wall preemptively adjust direction away from the wall. 
%
% 04/2016 by Benjamin Seibold, modified 04/2022 by Katrina Janeczko
...
for j = 1:ns % loop over steps
    ...
    ind = (X(:,1)<(ax(1)+1)&cos(D)<0)|... % who is within 1 of hitting a wall
        (X(:,1)>(ax(2)-1)&cos(D)>0); % horizontally
    D(ind) = pi-D(ind); % reverse x-direction
    ind = (X(:,2)<(ax(3)+1)&sin(D)<0)|... % who is hitting a wall
        (X(:,2)>(ax(4)-1)&sin(D)>0); % vertically
    ...
    for i = 1:n % loop over all agents
        agents_in_cell{cell_of_agent(i)}(end+1) = i; %makes list of all agents in each cell
    end
    % Determine list of agents in cell and adjacent cells
    i_L = (1:nc)-1+(mod(1:nc,ncx)==1); % cell index shifted left %Left neighbor of ledtmost cell is itself (CHANGE FOR PERIODIC BOUINDARY CONDS)
    i_R = (1:nc)+1-(mod(1:nc,ncx)==0); % cell index shifted right %Rifht neighbor of righttmost cell is itself (CHANGE FOR PERIODIC BOUINDARY CONDS)
    i_LD = max(i_L-ncx,1); % cell index shifted left and down
    i_D = max((1:nc)-ncx,1); % cell index shifted down %SAME
    i_RD = max(i_R-ncx,1); % cell index shifted right and down
    i_LU = min(i_L+ncx,ncx*(ncy-1)); % cell index shifted left and up
    i_U = min((1:nc)+ncx,ncx*(ncy-1)); % cell index shifted up %SAME
    ...
    for i = 1:n % loop over all agents
        ...
        if ~isempty(neighbors) % if there is at least one nearby agent
            ...
            % Attraction Zone
            ind_too_far = dist0>=2/3;
            angle_neighbors(ind_too_far) = ... % turn toward
                atan2(pos0(ind_too_far,2), ... % neighbors that are
                pos0(ind_too_far,1));   % of distance 2/3 to 1
